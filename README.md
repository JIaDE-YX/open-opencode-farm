...
官方免费的额度大头是按「出口 IP」独立算的（一个 IP 一天大概几百次调用，UTC 零点重置），外加每个账号 5 小时冷却。

- **不加技巧（单号单 IP 直连）**：一天几百次，用完就 429，干等 5 小时或等第二天。
- **加上这套（多号 × 多出口 × 1:1 绑定）**：有多少个独立出口 IP，就有多少个独立额度池——当前实例三四个机场 IP 再挂十来条住宅线路，粗算下来一天几千次起步；多账号轮换后，5 小时冷却也能互相错开，基本全天有额度可用。

**一句话：大约从「一天几百次」到「一天几千次」，量级差十倍左右。** 官方规则会变，以上只是个人使用过程中的大致感受，别当承诺。
